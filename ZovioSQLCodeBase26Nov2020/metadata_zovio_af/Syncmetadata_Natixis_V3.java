 ...
 * @author InkolluReddy
 */
public class Syncmetadata_Natixis_V3 {

    private static final Logger LOGGER = Logger.getLogger(Syncmetadata_Natixis_V3.class.getName());
    ...
    public String metadataSync(List<String> envMap, SystemManagerUtil smutill, String json, String folderName, int projectid) {
        ...
        try {
            ...
            appendHashforTable(mapSPecsLists);
            addSchemaNameforOrphantables(mapSPecsLists);
    ...
    public void sourceNamesSet(String Sourcetablename, List<String> envMap, MappingSpecificationRow mapSPecs, String mapName, String storprocName, String folderName, Map<String, String> sysEnvTableDetails, Map<String, Map<String, Integer>> tableColDetails, SystemManagerUtil smutill) {

        if (mapSPecs.getSourceTableName().split("\n").length == 1 && mapSPecs.getSourceTableName().split("\\.").length >= 4) {
            if (!sysEnvTableDetails.containsKey(mapSPecs.getSourceTableName().toUpperCase())) {
            ...
            if (sysEnvTableDetails.containsKey(Sourcetablename)) {
                String sysEnvDetails = sysEnvTableDetails.get(Sourcetablename);
                String[] details = sysEnvDetails.split("###");
                mapSPecs.setSourceSystemEnvironmentName(details[1]);
                mapSPecs.setSourceSystemName(details[0]);
            }
    ...
    public void targetNameSet(String Targettablename, List<String> envMap, MappingSpecificationRow mapSPecs, String mapName, String storprocName, String folderName, Map<String, String> sysEnvTableDetails, Map<String, Map<String, Integer>> tableColDetails, SystemManagerUtil smutill) {
        if (mapSPecs.getTargetTableName().split("\n").length == 1 && mapSPecs.getTargetTableName().split("\\.").length >= 4) {
            ...
            if (sysEnvTableDetails.containsKey(Targettablename)) {
                String sysEnvDetails = sysEnvTableDetails.get(Targettablename);
                String[] details = sysEnvDetails.split("###");
                mapSPecs.setTargetSystemEnvironmentName(details[1]);
                mapSPecs.setTargetSystemName(details[0]);
            }

        } else {
            if (Targettablename.split("\n").length > 1) {
                String[] tgttablecolumn = Targettablename.split("\n");
                List<String> tgtsystem = getTargetSystem(tgttablecolumn, mapName, envMap, mapSPecs, storprocName, folderName, sysEnvTableDetails, tableColDetails, smutill);
                List<String> tgteenv = getTargetEnv(tgttablecolumn, mapName, envMap, mapSPecs, storprocName, folderName, sysEnvTableDetails, tableColDetails, smutill);
            ...
            if (Targettablename.split("\n").length > 1) {
                ...
                String[] tgttabNamearr = Targettablename.split("\n");

                for (String trgetatbName : tgttabNamearr) {
    ...
    public List<String> getSourceSystem(String[] sourcetablename, String mapname, List<String> envMap, MappingSpecificationRow mapSPecs, String storprocName, String folderName, Map<String, String> sysEnvTableDetails, Map<String, Map<String, Integer>> tableColDetails, SystemManagerUtil smutill) {
        ...
        try {
            for (String sourceTab : sourcetablename) {
                ...
                //        updateTableDetails(envMap, sourceTab, smutill, sysEnvTableDetails, tableColDetails, mapSPecs.getSourceTableName());
                //     }
                if (!sysEnvTableDetails.containsKey(sourceTab)) {
    ...
    public List<String> getTargetSystem(String[] sourcetablename, String mapname, List<String> envMap, MappingSpecificationRow mapSPecs, String storprocName, String folderName, Map<String, String> sysEnvTableDetails, Map<String, Map<String, Integer>> tableColDetails, SystemManagerUtil smutill) {
        List<String> sourcesystem = new LinkedList<>();
        try {
            for (String sourceTab : sourcetablename) {
    ...
    public List<String> getTargetEnv(String[] sourcetablename, String mapname, List<String> envMap, MappingSpecificationRow mapSPecs, String storprocName, String folderName, Map<String, String> sysEnvTableDetails, Map<String, Map<String, Integer>> tableColDetails, SystemManagerUtil smutill) {
        List<String> sourceenv = new LinkedList<>();
        try {
            for (String sourceTab : sourcetablename) {
                if (!sysEnvTableDetails.containsKey(sourceTab)) {
    ...
    public static void removebraces(ArrayList<MappingSpecificationRow> mappingspec) {
        ...
        try {
            ...
            while (iter.hasNext()) {
                ...
                if (row.getSourceColumnName().toUpperCase().startsWith("CASE")) {
                    row.setSourceColumnName("");
                }
                if (row.getTargetColumnName().toUpperCase().startsWith("CASE")) {
    ...
    private void updateTableDetails(List<String> sysEnvDetails, String tableName, SystemManagerUtil smUtil, Map<String, String> sysEnvTableDetails, Map<String, Map<String, Integer>> tableColDetails, String sysEnvTableName) {
        ...
        for (int i = 0; i < sysEnvDetails.size(); i++) {
            ...
            try {
                ...
                LOGGER.log(Level.SEVERE, "Exception Occured at Syncmetadata_Natixix_V3 inside updateTableDetails()>>", e);
            }
        ...
        try {
            if (StringUtils.isNotEmpty(sysEnv)) {
                ...
                if (tableId != -1) {
                    ...
                    sysEnvTableDetails.put(tableName, sysEnv + "###" + tableId);
                    sysEnvTableDetails.put(sysEnvTableName.toUpperCase(), "value");
                }
            }
    ...
    public static List<String> getappendtargetTables(String[] targetTablename, String MapName) {
        ...
        try {
            for (String targetTab : targetTablename) {
                ...
                if (targetTab.toUpperCase().contains("RESULT_OF_") || targetTab.toUpperCase().contains("INSERT-SELECT") || targetTab.toUpperCase().contains("UPDATE-SELECT")) {
                    targetTab = targetTab + MapName;
                }
    ...
    public static void addSchemaNameforOrphantables(ArrayList<MappingSpecificationRow> mapspeclist) {
        ...
        for (MappingSpecificationRow mappingSpecificationRow : mapspeclist) {
            ...
            if (!mappingSpecificationRow.getSourceTableName().trim().contains("\\.") && !mappingSpecificationRow.getSourceTableName().toUpperCase().contains("INSERT-SELECT") && !mappingSpecificationRow.getSourceTableName().toUpperCase().contains("UPDATE-SELECT") && !mappingSpecificationRow.getSourceTableName().toUpperCase().contains("RESULT_OF") && !mappingSpecificationRow.getSourceTableName().toUpperCase().startsWith("#")) {
                ...
                if (srctabName.equalsIgnoreCase("PERSONNE")) {
                    int a = 10;
                }
                for (MappingSpecificationRow mappingSpecificationRow1 : mapspeclist) {
                    ...
                    if (dotsrctabName.split("\\.").length >= 2) {
                        if (dotsrctabName.equalsIgnoreCase("acces.Personne")) {
                            int b = 20;
                        }
            ...
            if (!mappingSpecificationRow.getTargetTableName().trim().contains("\\.") && !mappingSpecificationRow.getTargetTableName().trim().toUpperCase().contains("INSERT-SELECT") && !mappingSpecificationRow.getTargetTableName().trim().toUpperCase().contains("UPDATE-SELECT") && !mappingSpecificationRow.getTargetTableName().trim().toUpperCase().contains("RESULT_OF") && !mappingSpecificationRow.getTargetTableName().toUpperCase().startsWith("#")) {
                ...
                if (tgttabName.equalsIgnoreCase("PERSONNE")) {
                    int a = 10;
                }
                for (MappingSpecificationRow tgtmappingSpecificationRow1 : mapspeclist) {
                    ...
                    if (dotTgttabName.split("\\.").length >= 2) {
                        if (dotTgttabName.equalsIgnoreCase("acces.Personne")) {
                            int b = 20;
                        }
