 ...
 * @author Trinesh
 */
public class MappingCreator_PUMA {
    ...
    public static List<Mapping>jsMappings=new ArrayList();
...
//        String fileDirectoryPath = "D:\\Projects\\SNHU\\Test";
        ...
        String vendor = "mssql";
        MappingCreator_PUMA creator = new MappingCreator_PUMA();
    ...
    public static List<Mapping> createMappingForMsSQl(String dir1, String sysEnvDetails1, int projectId, int subjectId, String vendor,MappingManagerUtil maputil) {
...
//            TGSqlParser sqlparser = isQueryParsable(sqltext);
            ...
            DataFlowAnalyzer_PUMA dlineage = new DataFlowAnalyzer_PUMA(sqltext, fileName, dbVendor, false);
...
//            FileUtils.writeStringToFile(xmlFile, result, "UTF-8");
    ...
    public ArrayList<MappingSpecificationRow> getMappingSpecList(String inputFilePath, String[] sysenvDetails) {
        ...
        try {
            ...
            RelationAnalyzer_PUMA relationAnalyzer = new RelationAnalyzer_PUMA();
            ...
            for (File f : folder.listFiles()) {
                ...
                if (f.isFile()) {
                    if (f.getName().matches(pattern) || f.getName().matches(".*\\.dml")) {
                        ...
                        MappingCreator_PUMA mappingCreator = new MappingCreator_PUMA();
        ...
        try {
            File[] arr=folder.listFiles();
                for(int k=0;k<arr.length;k++){
                    if (arr[k].getName().matches(pattern) || arr[k].getName().matches(".*\\.dml")) {
                        System.out.println("------------- FileName is--->" + folder.getAbsolutePath() + "<----------------");
                        MappingCreator_PUMA mappingCreator = new MappingCreator_PUMA();
                        Mapping mapping = mappingCreator.getMappingObject(arr[k].getAbsolutePath(), sysEnvDetails, projectId, projectId);
                        mappinglist.add(mapping);
                    }
                }
